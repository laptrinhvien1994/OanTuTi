...
function PosCtrl($location, $ionicPosition, $ionicSideMenuDelegate, $ionicHistory, $timeout, $interval, $q, $scope, $http, $rootScope, AuthFactory, $state, $ionicPopover, $ionicPopup, $ionicModal, LSFactory, $ionicScrollDelegate, toaster, printer, $filter, hotkeys, Auth, $PouchDB) {
    ...
    var clientSyncID = localStorage.getItem('clientSyncID');
    if (clientSyncID == null) {
        clientSyncID = uuid.v1();
        localStorage.setItem('clientSyncID', clientSyncID);
    }
                        ...
                        for (var x = 0; x < $scope.tables.length; x++) {
                            if ($scope.tables[x].tableUuid == msg.tables[0].tableUuid) {
                                ...
                                if ($scope.tables[x].tableOrder.length > 0) {
                                    var orderIndex = null;
                                    //Trường hợp bàn đó đã có sẵn orders. Lỡ dùng for -> nên dùng Array.prototype.findIndex cho nhanh.
                                    ...
                                    //Nếu chưa có order này trong hóa đơn, trường hợp báo bếp order mới.
                                    if (orderIndex == null) {
    ...
    $scope.openTable = function (t, isInMenu) {
    ...
    $scope.pickProduct = function (item) {
        ...
        if ($scope.isInTable) {
            $scope.openTable($scope.tableIsSelected, true);
        }
    }
                ...
                if ($scope.isSync) {
                    ...
                    for (var x = 0; x < untrackedItem.length; x++) {
                        $scope.tableIsSelected.tableOrder[$scope.orderIndexIsSelected].saleOrder.logs.push(
                            new Log(untrackedItem[x].itemID, untrackedItem[x].itemName, "BB", untrackedItem[x].quantity, timestamp, clientSyncID, false));
                    }
        ...
        hotkeys.add({
            ...
            callback: function () {
                ...
                if (!$scope.isInTable && !$scope.onSearchField) {
                    $scope.selectItem($scope.quantityItemPerRow);
                }
                else if ($scope.isInTable && !$scope.onSearchField) {
                    $scope.selectItem($scope.quantityTablePerRow);
                }
            }
        });
